...
MODELS_INFO = {
    "X1-Y2-Z1": {
        "home": [0.472098, -0.415840, 0.789471] 
    },
    "X2-Y2-Z2": {
        "home": [0.480013, -0.853908, 0.789471] 
    },
    "X1-Y3-Z2": {
        "home": [0.475937, -0.634054, 0.789471] 
    },
    "X1-Y2-Z2": {
        "home": [0.678948, -0.424059, 0.789471] 
    },
    "X1-Y2-Z2-CHAMFER": {
        "home": [0.877965, -0.399189, 0.789471] 
    },
    "X1-Y4-Z2": {
        "home": [0.284295, -0.864617, 0.789471] 
    },
    "X1-Y1-Z2": {
        "home": [0.275572, -0.413132, 0.789471] 
    },
    "X1-Y2-Z2-TWINFILLET": {
        "home": [0.272887, -0.627295, 0.789471] 
    },
    "X1-Y3-Z2-FILLET": {
        "home": [0.682787, -0.635804, 0.789471] 
    },
    "X1-Y4-Z1": {
        "home": [0.881767, -0.622449, 0.789471] 
    },
    "X2-Y2-Z2-FILLET": {
        "home": [0.688769, -0.867164, 0.789471] 
    },
}
...
BUILDING = json.load(open(os.path.join(PKG_PATH, "building.json"), "r"))
...
def get_legos_pos(vision=False, strip=True):
...
def build(building, center=(0, 0, 0)):
    center = np.array(center) + (0, 0, 0.057/2)

    legos = get_legos_pos(vision=False, strip=False)
    legos.sort(reverse=False, key=lambda a: (a[1].position.x, a[1].position.y))

    def find_model(model_name):
        for i, (name, pose) in enumerate(legos):
            if get_model_name(name) == model_name:
                legos.pop(i)
                return name, pose

    controller.move_to(*DEFAULT_POS, DEFAULT_QUAT)

    for layer_name, layer in building.items():
        for block in layer:
            model_name = block["name"]
            target_pos = (block["x"],  block["y"],  block["z"]) + center
            target_quat = PyQuaternion(1,0,0,0) * DEFAULT_QUAT

            gazebo_model_name, model_pose = find_model(model_name)
            straighten(model_pose, gazebo_model_name)
            controller.move_to(z=center[2] + 0.1, blocking=False)

            controller.move_to(x=target_pos[0], y=target_pos[1], target_quat=target_quat)
            controller.move_to(z=target_pos[2])
            set_model_fixed(gazebo_model_name)
            open_gripper(gazebo_model_name)
            controller.move_to(z=center[2] + 0.1, blocking=False)
        center[2] += 0.057 - INTERLOCKING_OFFSET
...
if __name__ == "__main__":
    ...
    build(BUILDING, center=(0, 0, SURFACE_Z))
